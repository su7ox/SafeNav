...
#  Used to distinguish "legitimate cloud host"
#  from "shady residential/VPS host"
# ─────────────────────────────────────────────
MAJOR_HOSTING_ASNS = {
...
def _lookup_rdap(domain: str) -> dict:
    ...
    try:
        ...
        resp = requests.get(
            f"{rdap_url}domain/{extracted.domain}.{extracted.suffix}",
            timeout=5
        ).json()
...
def _lookup_ip_geo(ip: str) -> dict:
    ...
    try:
        fields = (
            "status,country,countryCode,regionName,"
            "city,isp,org,as,proxy,hosting"
        )
        resp = requests.get(
...
async def check_ip_intel(url: str) -> dict:
    """
    Full IP & origin intelligence for a URL.

    Returns:
        ip_address          — resolved IPv4/IPv6
        hosting_country     — "United States"
        hosting_country_code— "US"
        hosting_flag        — "🇺🇸"
        hosting_city        — "Ashburn"
        hosting_region      — "Virginia"
        isp                 — "Amazon.com, Inc."
        asn                 — "AS16509"
        asn_org             — "Amazon Web Services" (if known)
        is_major_host       — True if ASN is a recognized cloud/CDN provider
        is_vpn_or_proxy     — True if ip-api flags it
        registrant_country  — country where domain was registered (from RDAP)
        registrant_flag     — flag emoji for registrant country
        registrar           — "MarkMonitor, Inc."
        registration_date   — "2019-03-12"
        country_mismatch    — True if hosting ≠ registrant country
        warning_flags       — list of risk signals
    """
    report = {
        # IP / Hosting
        "ip_address": None,
    ...
    if ip:
        ...
        if geo:
            ...
            # ASN parsing: ip-api returns "AS16509 Amazon.com, Inc."
            asn_raw = geo.get("as", "")
            if asn_raw:
                parts = asn_raw.split(" ", 1)
                report["asn"]     = parts[0]                    # "AS16509"
                report["asn_org"] = MAJOR_HOSTING_ASNS.get(     # known name or raw
                    parts[0], parts[1] if len(parts) > 1 else None
                )
                report["is_major_host"] = parts[0] in MAJOR_HOSTING_ASNS

            # ── Risk signals from IP geo ──────────────────────────────────
            ...
            if asn in SUSPICIOUS_ASNS:
                report["warning_flags"].append(
                    ...
                    "frequently associated with phishing/abuse infrastructure"
                )
    ...
    if hcc and rcc and hcc != rcc:
        ...
        report["warning_flags"].append(
            f"Country Mismatch: Registered in {rcc} {country_flag(rcc)} "
            f"but hosted in {hcc} {country_flag(hcc)}"
        )
